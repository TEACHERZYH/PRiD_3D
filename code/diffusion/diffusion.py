...
class Diffusion:
    ...
    def to(self, device):
        """把扩散系数张量迁移到指定设备（GPU 训练必需）。

        Diffusion 不是 nn.Module，其系数张量默认留在 CPU；当 x0 在 GPU 上时
        用 GPU 索引会报 "indices should be either on cpu or on the same device"。
        """
    ...
    def refine(self, x0, cond, src, sup, K=3, t_refine=30, compliance_fn=None,
        ...
        参数:
            ...
            compliance_fn: 可选，callable(rho_np)->float，返回该密度场的柔度。
                ...
                偶尔破坏脆弱传力路径导致的柔度恶化）。
            bridge: 是否执行桥接投影。False 时跳过桥接，用于「只重构」对照。
            reconstruct: 是否执行低噪声重构。False 时循环只做桥接、不去噪，
                配合 K=1 即「只桥接一次」（one-shot），用于 2x2 因子对照。
                bridge=False 且 reconstruct=False 时 refine 为恒等映射。
        """
        ...
        for _ in range(K):
            if bridge:
                ...
                x_proj = x0  # no-bridge control: reconstruct without repairing connectivity

            if not reconstruct:
                # only-bridge control: repair connectivity without denoising
                x0 = x_proj
                continue
    ...
    def sample(self, cond, shape, ddim_steps=50, use_projection=False,
               ...
               x_init=None, t_start=None, trace_fn=None):
        ...
        参数:
            ...
            x_init: 可选初始状态 (B,1,D,H,W)。给定时不以随机噪声起步，用于
                "可行性吸引子"实验（从加噪后的破坏解出发做去噪）。
            t_start: 可选起始噪声水平（整数）。None 时取 timesteps-1。与 x_init
                配合：x_init 应为对应 t_start 的加噪结果。
            trace_fn: 可选回调 trace_fn(step_index, x0_pred, t)，记录去噪轨迹上
                每步的 x0 估计，用于可行性演化分析。默认 None（行为不变）。
        """
        device = cond.device
        b = shape[0]
        x_t = torch.randn(shape, device=device) if x_init is None else x_init
        t_hi = self.timesteps - 1 if t_start is None else int(t_start)
        times = torch.linspace(t_hi, 0, ddim_steps).long().to(device)
